...
class mAP_YOLO(YOLO):
    ...
    def generate(self):
        ...
        #   载入模型
        #---------------------------------------------------------#
        self.yolo_model = yolo_body(Input(shape=(None, None, 4)), num_anchors//2, num_classes)
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image_id, image, depth_image):
        ...
        if self.letterbox_image:
            ...
            boxed_depth_image = letterbox_image(depth_image, (self.model_image_size[1],self.model_image_size[0]))
        else:
            ...
            boxed_depth_image = depth_image.resize((self.model_image_size[1], self.model_image_size[0]), Image.BICUBIC)

        image_data = np.concatenate([boxed_image, np.expand_dims(boxed_depth_image, -1)], -1)
...
for image_idx in tqdm(range(len(lines))):
# for image_idx in range(len(lines)):
    line_annotation = lines[image_idx]
    line = line_annotation.split()[0]
    image_name = line.split('/')
    image_name = image_name[-1]
    image_id = image_name[4:9]

    image = Image.open(line)

    depth_image_path = total_depth_names[image_idx]
    depth_image = Image.open(depth_image_path)
    depth_image = depth_image.convert('L')

    # image_data = np.array(image, np.uint8)
    # depth_image_data = np.array(depth_image, np.uint8)
    # depth_image_data = np.expand_dims(depth_image_data, -1)

    # 开启后在之后计算mAP可以可视化
    # image.save("./input/images-optional/"+image_id+".jpg")
    yolo.detect_image(image_id, image, depth_image)
