...
def get_random_data(annotation_line, input_shape, max_boxes=100,
    ...
    image = image.resize((nw, nh), Image.BICUBIC)
    depth_image = depth_image.resize((nw, nh), Image.BICUBIC)
    ...
    new_depth_image = Image.new('RGB', (w, h), (128, 128, 128))
    new_depth_image.paste(depth_image, (dx, dy))
    ...
    def color_space_change(image_inner):
        x_inner = cv2.cvtColor(np.array(image_inner, np.float32) / 255, cv2.COLOR_RGB2HSV)

        x_inner[..., 0] += hue * 360
        x_inner[..., 0][x_inner[..., 0] > 1] -= 1
        x_inner[..., 0][x_inner[..., 0] < 0] += 1
        x_inner[..., 1] *= sat
        x_inner[..., 2] *= val
        x_inner[x_inner[:, :, 0] > 360, 0] = 360
        x_inner[:, :, 1:][x_inner[:, :, 1:] > 1] = 1
        x_inner[x_inner < 0] = 0
        return x_inner

    x = color_space_change(image)
    ...
    image_data = np.array(image_data * 255, np.uint8)
    depth_image_data = np.array(depth_image, np.uint8)
    image_data = np.concatenate([image_data, np.mean(depth_image_data, axis=-1, keepdims=True)], -1)
